 ...
 * the License.
 */
package com.wanxianbo.plugins.optimize;
...
public class JsqlParserCountOptimize implements ISqlParser {
    ...
    public SqlInfo parser(MetaObject metaObject, String sql) {
        ...
        try {
            ...
            List<Expression> groupBy = plainSelect.getGroupBy().getGroupByExpressions();
            List<OrderByElement> orderBy = plainSelect.getOrderByElements();

            // 添加包含groupBy 不去除orderBy
            if (CollectionUtils.isEmpty(groupBy) && !CollectionUtils.isEmpty(orderBy)) {
            ...
            if (distinct != null || !CollectionUtils.isEmpty(groupBy)) {
                return sqlInfo.setSql(SqlParserUtils.getOriginalCountSql(selectStatement.toString()));
            }
            // 包含 join 连表,进行判断是否移除 join 连表
            List<Join> joins = plainSelect.getJoins();
            if (optimizeJoin && !CollectionUtils.isEmpty(joins)) {
